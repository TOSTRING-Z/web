...
def zhu_ce_yan_zheng():
    if request.method == 'POST':
        yz = (session.get('yz') != request.form.get('yz'))
        user_name = request.form.get('username')
        password = request.form.get('password_1')
        zc_rs = pysql.zhu_ce_yan_zheng(user_name, password,yz)
        if zc_rs not in [None,0,'你已经注册过了哦。']:
...
def user_sel(user_id,user_sel):
    ...
    if user_sel == '动态':
        ...
        for i, foo in enumerate(data):
            ...
            d = {}
            d['main'] = \
            '<div id="'+foo.title+'" onclick="detail(this)">' \
                           '<div class="row">' \
                               '<div class="col-lg-12">' \
        ...
        for i, foo in enumerate(pysql.leaving_message_read(user_id)):
            ...
            '<div class="user-liuyan">' \
                        '<div class="row">' \
                            ...
                            '<span class="badge">&emsp;发表于:' + foo.time.strftime('%Y-%m-%d') + '</span>' \
                            '<a href="/deleteLeaving/'+str(foo.id)+'/'+str(user_id)+'"><i class="fa fa-trash-o" aria-hidden="true"></i></a></div>' \
                            '<br>' \
                            '<p>' + foo.content + '</p>' \
                            '<div class="light-a">' \
                        '</div>'
    ...
    if user_sel == '收藏':
        ...
        for i, foo in enumerate(pysql.user_collection_read(user_id)):
            d = {}
            d['main'] = '<div class="user-shouchang">' \
                '<div class="row">' \
                '<h4><a href="/lun_tan/detail/'+str(foo.title_id)+'" title="'+foo.title_name+'">' + foo.title_name + '</a></h4>' \
                ''+( '' if  user_id != str(session.get('user_id')) else '<a href="/deleteCollection/{0}/{1}"><i class="fa fa-trash-o" aria-hidden="true"></i></a>'.format(foo.title_id,user_id))+'' \
                '</div>' \
            '</div>'
            data_table_main.append(d)
        if data_table_main == []:
    ...
    if user_sel == '关注':
        ...
        for i, foo in enumerate(pysql.user_follow_read(user_id)):
            ...
            d['main'] = '<div class="user-guanzu">' \
                ...
                ''+( '' if  user_id != str(session.get('user_id')) else '<a href="/deleteFollow/{0}/{1}"><i class="fa fa-trash-o" aria-hidden="true"></i></a>'.format(foo.follow_id,user_id))+'' \
                '</div>' \
            '</div>'
            data_table_main.append(d)
        if data_table_main == []:
...
def lun_tan():
    if request.method == 'GET':
        ...
        if zi_yuan_search_type:
            ...
            filters = str(request.values.get('filters'))
            filter_arr = {}
            res = re.search('region:(.*?),', filters)
            if res:
                if res.groups()[0] != '':
                    filter_arr['region'] = res.groups()[0]
            res = re.search('year:(.*?),', filters)
            if res:
                if res.groups()[0] != '':
                    filter_arr['year'] = res.groups()[0]
            res = re.search('type:(.*?),', filters)
            if res:
                if res.groups()[0] != '':
                    filter_arr['type'] = res.groups()[0]
            result = pysql.zi_yuan_search(search,zi_yuan_search_type,sort,filter_arr)
            if result != {}:
                return render_template('zi_yuan/search_sourch.html',search = search,data = result)
            return render_template('zi_yuan/search_sourch.html',data = [],search = search)
        ...
        for i, foo in enumerate(data):
            ...
            for imgname in foo.imgName.split('|'):
                if imgname is not '' and img is '':
                    img = img + '<img style="height:150px;width;100%;object-fit: cover;" src="/static/public/img/timer.gif" imgPath="{0}!800-800" class="img-fluid"></img>'.format(imgname)
            d = {}
...
def set_cookie(user_id,type,title_id):
    if session.get('user_id'):
        ...
        resp.set_cookie('{0}data-up{1}'.format(user_id,title_id), type, max_age=24*60*60*30)
        return resp
    resp = Response("服务器返回信息")
...
def root():
    ...
    data = pysql.ziYuan()
    return render_template("root.html", data=data, data_time=data_time)
#资源
@app.route('/zi_yuan')
@app.route('/home')
def zi_yuan():
    data_time = pysql.time_tb() or []
    data = pysql.ziYuan()
    return render_template("zi_yuan/zi_yuan.html",data = data,data_time = data_time)
#资源server
...
def zi_yuan_read(start):
    ...
    return ''
#资源排行
@app.route('/zi_yuan/pai_hang')
def zi_yuan_pai_hang():
    data = pysql.paiHangRead()
    return render_template("zi_yuan/pai_hang.html",data = data)
...
def owoSubmit(source_id,come,num):
    ...
    if request.method == 'POST':
        if user_id:
            ...
            if num == 'detail':
                return redirect("/ziYuanDetail/{0}".format(source_id, come))
            return redirect("/ziYuanDetail/{0}/{1}/{2}".format(source_id,come,num))
        return render_template('deng_lu.html')
@app.route('/ziYuanDetail/<id>')
def ziyuan_detail(id):
    ...
    come = request.values.get('come') or '0'
    if come != '':
        if come == '0':
            result.path = result.path0
        if come == '1':
    ...
    return render_template("zi_yuan/detail.html", source_recommend=source_recommend, source_love=source_love,
                           data=detail,source = result,come = come)
#资源ziYuanDetail
...
def yz():
    id = random.randint(0, 7000000)
    session['yz'] = str(id)
    resp = Response("服务器返回信息")
    cookie_yz = request.cookies.get("yz")
    if cookie_yz not in ['',None,'None']:
        return 'err!'
    resp.set_cookie('yz', str(id), max_age=60*60)
    return resp
...
def mv():
    yhdm_iframe = {
        'youku':'//5.5252e.com/2/?url=https://v.youku.com/v_show/id_{}.html',#'https://player.baodai.org/ipsign/player.php?v={}_youku',
        ...
        'qq':'https://api.jiexi.la/?url=https://v.qq.com/x/page/{}.html',
        'qqc': 'https://api.jiexi.la/?url={}',
        'qiyi':'https://api.jiexi.la/?url=https://www.iqiyi.com/v_{}.html',#'https://player.yaogougou.com/ipsign/player.php?id={}&type=iqiyi',
        ...
        'qqy':'https://api.jiexi.la/?url=https://v.youku.com/v_show/id_{}.html',#'http://www.2gty.com/apiurl/yun.php?url=https://www.iqiyi.com/v_{}.html',#'https://player.yaogougou.com/ipsign/player.php?id=http://www.iqiyi.com/v_{}.html&type=vip',
        ...
        'letv':'https://api.flvsp.com/?type=letv&vid={}',
        'url':'https://api.jiexi.la/?url={}',
        'html':'{}'
    }
    qqdm_iframe = {
        'sinahd':'https://player.s1905.com/ipsign/player.php?v={}_sina',
        'pptv':'https://player.s1905.com/ipsign/player.php?v={}_pptv',
        'sohu':'https://player.s1905.com/ipsign/player.php?v={}_mysohu',
        'yuku':'https://api.jiexi.la/?url=https://v.youku.com/v_show/id_{}.html',#'https://player.s1905.com/ipsign/player.php?v={}_youku',
        'bilicid':'https://player.s1905.com/ipsign/player.php?v={}_bilicid',
        'bilibili': 'http://99hd.net/mp4/bilibili.html?id={}',
        'youku':'https://api.jiexi.la/?url=https://v.youku.com/v_show/id_{}.html',#'https://player.s1905.com/ipsign/player.php?v={}_youku',
        'qqold':'https://player.s1905.com/ipsign/player.php?v={}_qq&style=mp4',
        'qq': 'https://player.s1905.com/ipsign/player.php?v={}_qq&style=mp4',
        'qiyi': 'https://api.jiexi.la/?url=https://www.iqiyi.com/v_{}.html',
        'url': 'https://api.jiexi.la/?url={}',
        'html': '{}'
    }
    ...
    if request.method == 'GET':
        ...
        url = '/mv/api?type={}&uid={}'.format(type,uid)
        if 'quan.qq.com' in str(uid):
            response = requests.head(uid, allow_redirects=True)
            url = '{}'.format(response.url)
        return render_template('mv/deplayer.html',cur = '{}_{}'.format(source_id,num),url = url)
    return 'None'
