...
if __name__ == '__main__':
    ...
    for t in [17612]:
        ...
        if html_show:
            ...
            if title:
                year = innerText.first(html_show, '<dt>年份：</dt>.*?<a href=".*?">(.*?)</a></dd>', '')
                other_name = title
                type = '剧情,动作'
                region = innerText.first(html_show, '<dt class="hidden-xs">地区：</dt>.*?<a href=".*?">(.*?)</a></dd>', '')
                imgName = 'https://www.55cc.cc{}'.format(innerText.first(html_show, '<img class="img-responsives ff-img vod-pic" data-original="(.*?)" alt=".*?">',''))
                subtitle = innerText.first(html_show, '剧情介绍.*?<div class="vod-content">(.*?)<.*?>', '')
