...
def deng_lu_yan_zheng(user_name,password):
    ...
    try:
        ...
        session.close()
#注册验证
def zhu_ce_yan_zheng(user_name,password,yz):
    session = conn.dbsession()
    try:
        result_user = session.query(user).filter(user.user_name == user_name).first()
        if result_user:
...
def ziYuan():
    session = conn.dbsession()
    try:
        result = session.query(source).filter(not_(source.tag)).order_by(source.browse.asc()).all()
        data = {}
        num = {}
        i = 0
        for d in result:
...
def source_recommend_read(id):
    session = conn.dbsession()
    try:
        types = session.query(source.type).filter(source.id == id).scalar()
        if types:
            type = types.split(',')[random.randint(0, len(types.split(',')) - 1)]
            #count = session.query(func.count('*')).select_from(source).filter(text('FIND_IN_SET("{}",type)'.format(type))).order_by(source.browse.desc()).scalar()
            result = session.query(source).filter(text('FIND_IN_SET("{}",type)'.format(type))).order_by(source.browse.desc()).limit(100).offset(0).all()
        else:
            #count = session.query(func.count('*')).select_from(source).order_by(source.browse.desc()).limit(1000).offset(0).scalar()
            result = session.query(source).limit(100).offset(0).all()
        data = {}
        rand = []
        for i in range(20):
...
def paiHangRead():
    session = conn.dbsession()
    data = {}
    data['新番'] = session.query(source).filter(text('FIND_IN_SET("新番连载",tag)')).order_by(source.browse.desc())[:10]
    data['番组计划'] = session.query(source).order_by(source.browse.desc())[:10]
    data['国漫'] = session.query(source).filter(source.region == "大陆").order_by(source.browse.desc())[:10]
    data['日漫'] = session.query(source).filter(source.region == "日本").order_by(source.browse.desc())[:10]
    return data
#search
def zi_yuan_search(search,zi_yuan_search_type,sort,filter_arr):
    filter_str = '''{}{}{}'''.format('.filter(source.region == "{}")'.format(filter_arr['region']) if 'region' in filter_arr else '','.filter(source.year == "{}")'.format(filter_arr['year']) if 'year' in filter_arr else '','''.filter(text('FIND_IN_SET("{}",type)'))'''.format(filter_arr['type']) if 'type' in filter_arr else '')
    session = conn.dbsession()
    try:
        result = []
        base_search = ''''''
        if zi_yuan_search_type == 'all':
            base_search = '''session.query(source)'''
        if zi_yuan_search_type == 'source':
            base_search = '''session.query(source).filter(func.concat(source.title,source.other_name,source.subtitle,source.type).like("%{}%"))'''.format(search)
        if zi_yuan_search_type == 'tag':
            base_search = '''session.query(source).filter(text('FIND_IN_SET("{}",tag)'))'''.format(search)
        if zi_yuan_search_type == 'type':
            base_search = '''session.query(source).filter(text('FIND_IN_SET("{}",type)'))'''.format(search)
        if zi_yuan_search_type == 'year':
            base_search = '''session.query(source).filter(source.year == "{}")'''.format(search)
        if zi_yuan_search_type == 'region':
            base_search = '''session.query(source).filter(source.region == "{}")'''.format(search)
        com = '''{}{}.order_by(source.{}.desc()).all()'''.format(base_search, filter_str, sort if sort else 'year')
        result = eval(com)
        data = {}
        for i, d in enumerate(result):
            d.subtitle = re.sub('(<img.*?>.*?</.*?>|<iframe.*?>.*?</.*?>|<video.*?>.*?</.*?>|<.*?>)','',str(d.subtitle))
            data[i] = {}
            data[i]['id'] = str(d.id)
            data[i]['title'] = str(d.title)
            data[i]['subtitle'] = (str(d.subtitle)[0:70] + '[...]') if len(str(d.subtitle)) > 69 else str(d.subtitle)
            data[i]['imgName'] = d.imgName
        return data
    except Exception as e:
        print('search:{}'.format(e))
        session.rollback()
        session.close()
    finally:
        session.close()
        ...
        for d in result:
            try:
                ...
                for index,tag in enumerate(d.time.split(',')):
                    ...
                    d.subtitle = re.sub('(<img.*?>.*?</.*?>|<iframe.*?>.*?</.*?>|<video.*?>.*?</.*?>|<.*?>)', '',d.subtitle)
                    data[tag][i] = {}
                    data[tag][i]['number'] = len(str(d.path0).split('@@'))-1
                    data[tag][i]['id'] = d.id
                    data[tag][i]['title'] = d.title
                    data[tag][i]['subtitle'] = (d.subtitle[0:70] + '[...]') if len(d.subtitle) > 69 else d.subtitle
                    data[tag][i]['user_id'] = d.user_id
                    data[tag][i]['up_num'] = d.up_num
                    data[tag][i]['comment'] = d.comment
                    data[tag][i]['browse'] = d.browse
                    data[tag][i]['time'] = d.time.split(',')
                    data[tag][i]['num'] = str(d.path0).split('|')[-1]
                    data[tag][i]['imgName'] = d.imgName
                    i += 1
            except Exception as e:
                print(e)
...
def fen_lei():
    ...
    try:
        ...
        for key,val in enumerate(data['类型']):
            res['类型'].append('<a title="{}" class="fenLenSpan" href="/lun_tan?zi_yuan_search_type=type&search={}&sort=year&filters=region:,year:,type:,">{}</a>'.format(
                val, val, val))
        for key,val in enumerate(data['时间']):
            res['时间'].append('<a title="{}" class="fenLenSpan" href="/lun_tan?zi_yuan_search_type=year&search={}&sort=year&filters=region:,year:,type:,">{}</a>'.format(
                val, val, val))
        return res
    except:
        session.rollback()
